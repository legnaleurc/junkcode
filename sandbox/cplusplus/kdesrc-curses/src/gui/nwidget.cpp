...
NWidget::Private::~Private () {
    std::for_each(std::begin(this->children), std::end(this->children), [](NWidget * c)->void {
        delete c;
    });
}
...
void NWidget::resize (int rows, int cols) {
    ...
    // update children
    std::for_each(std::begin(this->p_->children), std::end(this->p_->children), [rows, cols](NWidget * w)->void {
...
boost::signals2::signal<void (int)> & NWidget::keyPressed () {
