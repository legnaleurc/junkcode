...
( function() {
	var __extention__ = {
		createTag: function( F ) {
			return this.replace( eval( 'RubyText.Filter.' + F ), '<ruby><rb>$1</rb><rp>$2</rp><rt>$3</rt><rp>$4</rp></ruby>' );
		},
		...
		clearText: function( F ) {
			return this.replace( eval( 'RubyText.Filter.' + F ), '$1' );
		},

		clearBase: function( F ) {
			return this.replace( eval( 'RubyText.Filter.' + F ), '$3' );
		},

		rubyTransform: function() {
			return this.replace( RubyText.Filter.ja.HGYouon, function( m0, offset ) {
	...
	for( var property in __extention__ )
		String.prototype[property] = __extention__[property];
} )();
